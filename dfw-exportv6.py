       ...
       for rule in ruled['results']:
         ...
         if rule.get('sources'):
            if 'target_id' in rule['sources'][0]:
                ...
                if 'IPSet' in rule['sources'][0]['target_type']:
                    ipsets = nsx.get('https://' + nsx_manager + '/api/v1/ip-sets/' + rule['sources'][0]['target_id'])
                        # Convert JSON response ipset rule to python dict ipsetd
                    ipsetsd = json.loads(ipsets.text)
                    try:
                     this['sources includes'] = [ipsetsd][0]['ip_addresses']
                    except:
                     this['sources includes'] = ' '
